...
export const Navbar: React.FC = () => {
  ...
  const mainPages: { id: PageTab; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
    { id: 'universe', label: '1. Architecture & Bio', icon: Home },
    { id: 'projects', label: '2. Projects & Systems', icon: FolderGit2 },
    { id: 'journey', label: '3. Journey & Credentials', icon: Milestone },
    { id: 'all', label: 'Full System View', icon: Layers },
  ];

  const themeOptions: { key: CosmicTheme; label: string; color: string }[] = [
    { key: 'cyan', label: 'Quantum Cyan', color: '#00f2fe' },
    { key: 'amethyst', label: 'Compiler Violet', color: '#c084fc' },
    { key: 'solar', label: 'Terminal Amber', color: '#fbbf24' },
    { key: 'aurora', label: 'Kernel Emerald', color: '#34d399' },
  ];

  return (
    <header id="main-navigation" className="sticky top-0 z-30 w-full bg-slate-950/80 backdrop-blur-2xl border-b border-slate-800/80 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[11px] text-cyan-300 font-mono flex items-center gap-1.5 font-medium">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 bg-slate-950/70 backdrop-blur-xl p-1.5 rounded-2xl border border-slate-800/80">
          {mainPages.map(page => {
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="p-2 rounded-xl bg-slate-950/70 hover:bg-slate-900/80 text-slate-200 hover:text-white border border-slate-700/60 backdrop-blur-xl transition-colors flex items-center gap-1.5 text-xs font-mono cursor-pointer"
            >
            ...
            {themeDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 rounded-2xl bg-slate-950/90 backdrop-blur-2xl border border-slate-700/70 p-2 shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 space-y-1">
                <div className="px-2 py-1 text-[10px] font-mono uppercase text-slate-300 font-bold">
                  System Palettes
                </div>
                {themeOptions.map(th => (
                  <button
                  ...
                  >
                    ...
                    {cosmicTheme === th.key && <span className="text-[10px] text-cyan-300 font-bold">Active</span>}
                  </button>
                ))}
              </div>
            )}
          ...
          {viewMode === '3d' && (
            <button
              ...
              title="Explore 3D Software Architecture topology"
            >
              <Compass className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">{is3DInteractiveOverlay ? 'Exit 3D' : '3D Explore'}</span>
            </button>
          )}
          ...
          <button
            ...
            title="Toggle between 3D software architecture canvas and 2D documentation"
          >
            {viewMode === '3d' ? <Sparkles className="w-3.5 h-3.5 text-cyan-300" /> : <Globe className="w-3.5 h-3.5 text-indigo-300" />}
