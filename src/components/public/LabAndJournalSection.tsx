...
export const LabAndJournalSection: React.FC = () => {
  ...
  return (
    <section id="journal" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-900">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-950/60 border border-cyan-500/30 text-xs font-mono text-cyan-300 mb-3">
            ...
            <span>Digital Archive & Experimental Lab</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white font-display tracking-tight">
            Lab Experiments & Engineering Journal
          </h2>
          <p className="mt-2 text-slate-400 text-sm sm:text-base max-w-xl">
            A repository of architectural explorations, performance optimizations, and technology advisory reflections.
          </p>
        </div>

        {/* Category Tabs */}
        <div className="flex gap-2 p-1 rounded-2xl bg-slate-900/90 border border-slate-800">
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPosts.map(post => (
          <article
            ...
            className="cursor-pointer group flex flex-col justify-between rounded-3xl bg-slate-900/50 border border-slate-800 hover:border-cyan-500/50 transition-all duration-300 p-6 backdrop-blur-sm hover:-translate-y-1 hover:shadow-[0_0_25px_rgba(56,189,248,0.12)]"
          >
      ...
      {selectedPost && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/85 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-slate-900 border border-cyan-500/40 rounded-3xl p-6 sm:p-8 space-y-6 text-slate-300 shadow-2xl">
            <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-800">
              <div className="space-y-1.5">
                <span className="px-2.5 py-0.5 rounded-full bg-cyan-950 border border-cyan-500/30 text-xs font-mono text-cyan-300 uppercase">
                ...
                <div className="text-xs font-mono text-slate-400">
                  Published: {selectedPost.date} by Bilal Hassan Mussa
                </div>
              </div>
            ...
            <div className="text-sm leading-relaxed whitespace-pre-line text-slate-300 font-sans space-y-4">
            ...
            <div className="pt-4 border-t border-slate-800 flex flex-wrap items-center justify-between gap-2">
              <div className="flex flex-wrap gap-1.5">
                {selectedPost.tags.map(t => (
                  <span key={t} className="px-2.5 py-1 rounded-lg bg-slate-950 border border-slate-800 text-[11px] font-mono text-cyan-400 flex items-center gap-1">
